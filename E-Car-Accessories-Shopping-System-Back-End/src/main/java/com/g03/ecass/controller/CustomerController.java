...
import com.g03.ecass.dto.ProductCartId;
...
@RestController
@RequestMapping("/customers/services")
@CrossOrigin
public class CustomerController {
	...
	public ResponseDTO<?> getAllProductListlowtohigh(@PathVariable int categoryId) {
		...
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Product List Added",
					custService.getAllProductlowtohigh(categoryId));
		} catch (RuntimeException e) {
	...
	public ResponseDTO<?> getAllProductListhightolow(@PathVariable int categoryId) {
		...
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Product List Added",
					custService.getAllProducthightolow(categoryId));
		} catch (RuntimeException e) {
	...
	@GetMapping("/product")
	public ResponseDTO<?> getProductAll() {
		System.out.println("in get Product all: ");
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Product Added", custService.getProductAll());
		} catch (RuntimeException e) {
			System.out.println("err in getProductById : " + e);
			return new ResponseDTO<>(HttpStatus.INTERNAL_SERVER_ERROR, "Product Not Found", null);
		}
	}
	...
	public ResponseDTO<?> addProductToCart(@RequestBody ProductCartId productCartId) {
		...
		try {
			Products product = custService.getProductById(productCartId.getProductId());
			Cart c = new Cart();
			c.setDescription(product.getDescription());
			c.setDiscountOffer(product.getDiscountOffer());
			c.setFinalPrice(product.getFinalPrice());
			c.setProductModel(product.getProductModel());
			c.setManufacture(product.getManufacture());
	...
	public ResponseDTO<?> getProductListByName(@PathVariable String productName) {
		...
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Product List Added",
					custService.getProductListByName(productName));
		} catch (RuntimeException e) {
	...
	public ResponseDTO<?> addorderDetails(@PathVariable int userId, @PathVariable int orderId) {
		...
		try {
			return new ResponseDTO<>(HttpStatus.OK, "order Details Added",
					custService.addOrdersDetails(userId, orderId));
		} catch (RuntimeException e) {
	...
	public ResponseDTO<?> addpayment(@RequestBody PaymentDTO paymentDTO) {
		...
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Payment Successfully", custService.addPayment(paymentDTO));
		} catch (RuntimeException e) {
	...
	public ResponseDTO<?> cancelOrder(@PathVariable int orderId) {
		System.out.println("in getOrdersList: " + orderId);
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Order List Added", custService.cancelOrder(orderId));
		} catch (RuntimeException e) {
			System.out.println("err in getOrdersList : " + e);
			return new ResponseDTO<>(HttpStatus.INTERNAL_SERVER_ERROR, "Order List Not Added", null);
		}
	...
	public ResponseDTO<?> getOrdersDetailsList(@PathVariable int orderId) {
		...
		try {
			return new ResponseDTO<>(HttpStatus.OK, "Order Details List Added",
					custService.getOrdersDetailsList(orderId));
		} catch (RuntimeException e) {
